...
public class PlaybackThread {
      int mSAMPLE_RATE = 8500;
    private static final String LOG_TAG = PlaybackThread.class.getSimpleName();

    public PlaybackThread(int SAMPLE_RATE,short[] samples, PlaybackListener listener) {
    ...
    private void play() {
        int bufferSize = AudioTrack.getMinBufferSize(mSAMPLE_RATE, AudioFormat.CHANNEL_OUT_MONO,
                AudioFormat.ENCODING_PCM_16BIT);
        if (bufferSize == AudioTrack.ERROR || bufferSize == AudioTrack.ERROR_BAD_VALUE) {
            bufferSize = mSAMPLE_RATE * 2;
        }
        ...
        audioTrack.setPlaybackPositionUpdateListener(new AudioTrack.OnPlaybackPositionUpdateListener() {
            ...
            public void onPeriodicNotification(AudioTrack track) {
                if (mListener != null && track.getPlayState() == AudioTrack.PLAYSTATE_PLAYING) {
                    mListener.onProgress((track.getPlaybackHeadPosition() * 1000) / mSAMPLE_RATE);
                }
            }
            @Override
        ...
        audioTrack.setPositionNotificationPeriod(mSAMPLE_RATE / 30); // 30 times per second
